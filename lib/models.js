...
function models(deviceModels) {
	...
	var _getDeviceModel = function(brand, model) {
		if (deviceModels[brand] && deviceModels[brand][model]) {
			return deviceModels[brand][model];
		}
	};
	...
	var _ref = function (brand, model) {
		...
		for (var i=DEPTH; i>0; i--) {
			device = _getDeviceModel(tmpB, tmpM) || {};
			...
			if (device.brand && device.model) {
				if (_getDeviceModel(device.brand, device.model)){
					tmpB = device.brand;
					tmpM = device.model;
				}
				else {
					tmpB = device.brand;
					tmpM = device.model;
					if (!deviceModels[tmpB]) {
						deviceModels[tmpB] = {};
					}
					deviceModels[tmpB][tmpM] = device;
					return;
				}
			}
		}
		console.log('Recursion Issue: "' + brand + '" "' + model + '"');
